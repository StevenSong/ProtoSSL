...
class StreamingAudioWaveforms:
    ...
    def _random_crop_or_pad(self, x: torch.Tensor, target_len: int) -> torch.Tensor:
        cur_len = x.shape[1]
    ...
    def _load_waveform(self, i: int) -> torch.Tensor:
        fpath = self.wav_paths[i]
        x, source_sr = torchaudio.load(str(fpath))
    ...
    def sample_view(self, i: int, clip_seconds: float | None = None) -> torch.Tensor:
        ...
        target_len = self.target_len if clip_seconds is None else int(self.sampling_rate * clip_seconds)

        x = self._random_crop_or_pad(x, target_len=target_len)
        x = self._apply_train_augmentations(x)
...
class AudioSetDataset(BaseECGDataset):
    def __init__(
        ...
        *,
        ...
        ytid_codes, _ = pd.factorize(df["YTID"], sort=True)
        patient_ids = torch.as_tensor(ytid_codes, dtype=torch.long)
        ecg_ids = torch.arange(len(df), dtype=torch.long)
        ...
        assert self.patient_ids.shape[0] == len(self.waveforms)
        assert self.patient_ids.shape[0] == self.ecg_ids.shape[0]
        assert self.patient_ids.shape[0] == self.labels.shape[0]
