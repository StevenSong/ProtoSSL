...
#The below is adapted from the original HTS-AT repo: https://github.com/RetroCirce/HTS-Audio-Transformer/
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        #adjust reshaping to avoid destroying partial prototype mechanism

        if isinstance(self.spec_size, tuple):
            self.spec_h, self.spec_w = self.spec_size
        else:
            self.spec_h, self.spec_w = self.spec_size, self.spec_size

        self.freq_ratio = self.spec_h // self.config.mel_bins
        ...
        self.patch_embed = PatchEmbed(
            img_size=(self.spec_h, self.spec_w),
        ...
        if self.config.enable_tscam:
            ...
            #     padding = (0,1)
            # )
            SF = self.spec_h // (2 ** (len(self.depths) - 1)) // self.patch_stride[0]
            if self.freq_ratio > 1:
                SF = SF // self.freq_ratio
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            H0, W0 = self.patch_embed.grid_size
            SF = H0 // (2 ** (len(self.depths) - 1))
            ST = W0 // (2 ** (len(self.depths) - 1))
            ...
            H0, W0 = self.patch_embed.grid_size
            Hf = H0 // (2 ** (len(self.depths) - 1))
            Wf = W0 // (2 ** (len(self.depths) - 1))

            fpx = x.permute(0, 2, 1).contiguous().reshape(B, C, Hf, Wf)
...
## Below is new method required for partial prototypes ##
    def forward_feature_map(self, x):
        """
        Return the PRE-GROUPING 2D latent feature map for prototype learning.

        Input:
            x: waveform tensor before frontend, same as forward() expects
        ...
        x = self.reshape_wav2img(x)  # now geometry-preserving rectangular image
        ...
        # x = x.permute(0, 2, 1).contiguous().reshape(B, C, Hf, Wf)  # (B, C, F, T)
        # return x
        B, N, C = x.shape
        ...
        x = x.permute(0, 2, 1).contiguous().reshape(B, C, Hf, Wf)  # (B, C, F, T)
        return x
## Above is new method required for partial prototypes ##
    ...
    def reshape_wav2img(self, x):
        """
        Geometry-preserving version:
        - keep frequency as vertical axis
        - keep time as horizontal axis
        - resize directly to the configured rectangular image size
        """
        B, C, T, F = x.shape  # x is (B, 1, time, mel)

        target_F = self.spec_h
        target_T = self.spec_w

        if T != target_T or F != target_F:
            x = nn.functional.interpolate(
                x,
                size=(target_T, target_F),   # still (time, freq) at this point
                mode="bicubic",
                align_corners=True,
            )

        # convert to image layout: vertical=freq, horizontal=time
        x = x.permute(0, 1, 3, 2).contiguous()  # (B, 1, F, T)
        return x
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False):# out_feat_keys: List[str] = None):
        ...
        if infer_mode:
            ...
            if self.training:
                ...
                max_pos = max(0, x.shape[2] - self.spec_w)
                cur_pos = random.randint(0, max_pos) if max_pos > 0 else 0
                ...
                max_pos = max(0, x.shape[2] - self.spec_w)
                step = self.spec_w
                for cur_pos in range(0, max_pos + 1, step):
            ...
            if x.shape[2] > self.spec_w:

                if self.training:
                    
                    #x = self.crop_wav(x, crop_size=self.freq_ratio * self.spec_size)
                    x = self.crop_wav(x, crop_size=self.spec_w)
