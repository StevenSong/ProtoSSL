...
class AudioSetContrastiveWrapperDataset(Dataset):
    def __init__(
        self,
    ...
    def __getitem__(self, i: int) -> dict[str, torch.Tensor]:
        ...
        ret = {"source_id": base1["source_id"]}

        if self.pair_mode in {"cola", "cola+clar"}:
            assert isinstance(self.ds.waveforms, StreamingAudioWaveforms)
            ret["x1"] = self.ds.waveforms.sample_view(
                i, clip_seconds=self.cola_view_seconds
            )
            ret["x2"] = self.ds.waveforms.sample_view(
                i, clip_seconds=self.cola_view_seconds
            )
