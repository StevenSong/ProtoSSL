...
from ..layers._panns_backbones import (
    Cnn6,
    Cnn10,
    Cnn14,
    Cnn14_8k,
...
class PANNSEncoder(BaseEncoder):
    ...
    The backbone itself is responsible for defining:
      ...
      - forward(x): pooled embedding output computed from that same map
    """
    ...
    def __init__(
        ...
        *,
        ...
        if backbone_type not in PANNS_MODEL_REGISTRY:
            raise ValueError(
                f"Unknown PANNs model '{backbone_type}'. "
                f"Available: {sorted(PANNS_MODEL_REGISTRY.keys())}"
            )

        self.model_name = backbone_type
        self.sample_rate = sample_rate
        self.clip_seconds = clip_seconds
        self.target_len = int(sample_rate * clip_seconds)

        model_cls = PANNS_MODEL_REGISTRY[backbone_type]
        ...
        if not hasattr(self.model, "emb_dim"):
            raise ValueError(
                f"{backbone_type} must define self.emb_dim in _panns_backbones.py"
            )
    ...
    def logmel_for_viz(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not hasattr(self.model, "spectrogram_extractor") or not hasattr(
            self.model, "logmel_extractor"
        ):
            raise ValueError(
                f"logmel_for_viz not supported for PANNS model {self.model_name}"
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(out, dict):
            ...
            # clipwise output is the output relative to the classifier?
            # we just want the embedding?
