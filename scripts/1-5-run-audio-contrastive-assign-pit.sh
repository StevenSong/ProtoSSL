...
EXP_NAME="pass-audioset-ssl-assign"
...
srun python -m pass_pclr.trainer \
    ...
    "${COMMON_ARGS[@]}" \
    --data.contrastive_pair_mode $PAIR_MODE \
    --data.cola_view_seconds $COLA_VIEW_SECONDS \
    --model.init_args.contrastive_pair_mode $PAIR_MODE \
    --model.init_args.cola_loss_weight $COLA_LOSS_WEIGHT \
    --model.init_args.clar_loss_weight $CLAR_LOSS_WEIGHT \
    --model.init_args.koleo_loss_weight $KOLEO_LOSS_WEIGHT \
