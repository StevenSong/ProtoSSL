...
def evaluate_audioset(
    *,
    ...
    metrics["Multilabel Averaged"]["mAP"] = float(np.mean(ap_vals)) if len(ap_vals) > 0 else np.nan
    metrics["Multilabel Averaged"]["mAUC"] = float(np.mean(auc_vals)) if len(auc_vals) > 0 else np.nan
...
def main(
    *,  # enforce kwargs
    ...
    sampling_rate = get_eval_sampling_rate(dataset_path)
